...
func withKeyFile(t *testing.T, keyname string, callback func(string)) {
	...
	err = os.WriteFile(path.Join(tmpDir, "ssh"), []byte("#!/bin/bash\n"+
		"ssh -o \"UserKnownHostsFile=/dev/null\" -o \"StrictHostKeyChecking=no\" -o \"IdentitiesOnly=yes\" -o \"PubkeyAcceptedAlgorithms=+ssh-rsa\" -i \""+keyFile+"\" \"$@\""), 0700)
	assert.NoError(t, err)

	//Setup ssh wrapper
	os.Setenv("GIT_SSH", path.Join(tmpDir, "ssh"))
	os.Setenv("GIT_SSH_COMMAND",
		"ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o IdentitiesOnly=yes -o PubkeyAcceptedAlgorithms=+ssh-rsa -i \""+keyFile+"\"")
